...
const buildObject = (vm, callback) => mapValues(vm.$options.forms, item => callback(isFunction(item) ? item.bind(vm)() : item));

// Convert schema object to data object
const createDataFromSchema = form => mapValues(omitBy(form, isFunction), item => {
...
const createValidationsFromSchema = form => omitBy(mapValues(form, item => isPlainObject(item) ? createValidationsFromSchema(item) : isFunction(item) ? item : null), item => item === null || isPlainObject(item) && !Object.keys(item).length);
...
var index = (Vue => {
  ...
  let methods = {
    ...
    reset(name) {
      ...
      set(this, name, createDataFromSchema(this.$options.forms[name]));
    },
  ...
  Vue.mixin({
    ...
    beforeCreate() {
      ...
      this.$form = name => mapValues(this.$forms, callback => (...args) => callback(name, ...args));
    }
  });
});
